...
export default function PortalPage({ params }: PortalPageProps) {
  ...
  const { data: quotes, error, isLoading } = useSWR(
    resolvedParams ? `/portal/clients/${resolvedParams.cid}/reports/${resolvedParams.rid}/quotes` : null,
    fetcher
  );
  ...
  const handleNext = () => {
    if (selectedQuoteIndex !== null && quotes && selectedQuoteIndex < quotes.length - 1) {
      setSelectedQuoteIndex(selectedQuoteIndex + 1);
    }
  };

  const selectedQuote = selectedQuoteIndex !== null && quotes ? quotes[selectedQuoteIndex] : null;
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-gray-100">
          ...
          <div className="flex items-center justify-center py-20">
            <div className="text-center">
              ...
              <p className="text-gray-500 text-sm mt-2">
                This report doesn&apos;t have any public quotes yet.
              </p>
            </div>
          </div>
        )}
      ...
      <QuoteDrawer
        ...
        hasNext={selectedQuoteIndex !== null && quotes && selectedQuoteIndex < quotes.length - 1}
      />
    </div>
  );
}